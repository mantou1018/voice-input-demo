...
export function PositionPickerSheet({
  ...
  const selectedCount = positionPickerState.selectedItems.length;
        ...
        <div className="flex min-h-0 flex-1">
          ...
          <div className="flex-1 overflow-y-auto bg-[#f7f7f7]">
            {selectedCategory.options.map((option) => {
              const isSelected = positionPickerState.selectedItems.some((item) => item.option === option);
              return (
                <button
                  ...
                  type="button"
                >
                  <span className={isSelected ? 'text-[#ff3b66]' : ''}>{option}</span>
                  <span
